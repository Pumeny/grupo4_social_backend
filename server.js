...
import { MongoClient, ObjectId, ObjetId }  from 'mongodb';
...
const getAgricultores = ('/agricultores') => {
...
function getAgricultor(/agricultor/:id) => {
    return new Promise ((resolve, reject) => {
...
app.get('/index.html/:id/agricultores', async (req, res) => {
    ...
    try {
        letAgriculturesPorZonas = await db.collection('agricultores').find({zonaId: new ObjectId(zonasId) }).toArray();
